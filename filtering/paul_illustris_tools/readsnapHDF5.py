...
def read_block(filename, block, parttype=-1, no_mass_replicate=False, fill_block="", slab_start=-1, slab_len=-1, ids=-1, verbose=False, multiple_files=False):
        ...
        if (block in datablocks.keys()) or any(block in val for val in datablocks.values()):
                block_key = [key for key, value in datablocks.items() if block in value][0]
                block_name=datablocks[block_key][0]
                dim2=datablocks[block_key][1]
                first=True
                if (verbose):
